...
class RBF:
    ...
    def __init__(self, train_data: np.ndarray, units: int = 20, rho1: int = 1e-4,
                 rho2: int = 1e-4, sigma: float = 1):
        """
        :param train_data
        :param labels
        """
    ...
    def evaluate_loss(self, train_data: np.ndarray, labels: np.ndarray, centroids: np.ndarray = None) -> tuple[
        ndarray, float]:
        """
        :param train_data:
        :param labels:
        ...
        if centroids is not None:
            phi_mat = self.interpolation_mat(train_data=train_data, centroids=centroids)

        phi_mat = self.interpolation_mat(train_data=train_data, centroids=self.centroids)
        ...
        cross_entropy += self.rho2 * np.linalg.norm(self.centroids) ** 2
    ...
    def backprop(self, upstream_gradient: np.ndarray, phi_mat: np.ndarray) -> np.ndarray:
        ...
        np.divide(self.gradient, phi_mat.shape[0], out=self.gradient)
    ...
    def gradient_check(self, labels: np.ndarray, epsilon: int = 0.01):

        output_plus = np.zeros(shape=self.centroids.size, dtype=np.float32)
        centers_flatten = np.reshape(self.centroids, -1)
        for elem in range(len(centers_flatten)):
            ...
            output_plus[elem] = self.evaluate_loss(self.x, labels, current_params_plus)[0]

        output_minus = np.zeros(shape=self.centroids.size, dtype=np.float32)
        for elem in range(len(self.centroids)):
            ...
            # start evaluate loss pipeline
            output_minus[elem] = self.evaluate_loss(self.x, labels, current_params_minus)[0]
        ...
        gradient = self.evaluate_loss(self.x, labels, centers_flatten)[1]
    ...
    def fit(self, labels: np.ndarray, tol: float = 1e-2, epoch: int = 400):

        k = 0
        conv_count = 0
        while True:
            loss, gradient, phi_mat, out = self.evaluate_loss(self.x, labels)

            gradient_weights = np.dot(phi_mat.T, (out - labels))
            ...
            alpha = self.armijo_linesearch(self.x, labels, gradient, self.centroids)
            self.centroids = self.centroids - gradient
    ...
    def armijo_linesearch(self, train_data: np.ndarray, labels: np.ndarray, gradient: np.ndarray, x_0: np.ndarray,
                          alpha: float = 1.0, beta: float = 0.5, c1: float = 1e-3, max_iters: int = 100):

        direction = - gradient
        loss = self.evaluate_loss(train_data, labels)[0]

        for _ in range(max_iters):
            x_next = x_0 + alpha * direction
            loss_next = self.evaluate_loss(train_data, labels, centroids=x_next)[0]
