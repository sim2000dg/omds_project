...
class GaussianSVMComplete(GaussianSVM):
    def smo_fit(
        ...
        current_check = np.zeros_like(current_gradient)  # Initialize array we are taking max and min over
        dual_vars = np.zeros_like(current_gradient)  # Dual variables array
        grad_update = np.zeros_like(current_gradient)  # allocate array for gradient update computation
        set_r = labels == 1  # initialize mask for set R
        set_s = labels == -1  # initialize mask for set S
        gram = rbf_kernel(train_data, gamma=self.gamma)  # compute Gram matrix of inner products
        gram *= np.outer(labels, labels)  # Multiply each entry by y^iy^j
        it = 0  # Iteration counter

        while True:
            if np.sum(set_r) * np.sum(set_s) == 0:  # Check whether one of the two sets is empty, in that case break
                break
            np.multiply(current_gradient, labels, out=current_check)  # Compute -nabla_alpha(alpha^k)/y
            ...
            if current_check[r_pick] - current_check[s_pick] < tol:  # Condition for termination, KKT violation
                break
            ...
            rel_gram = gram[np.ix_([r_pick, s_pick], [r_pick, s_pick])]  # Entries of the hessian related to sel. vars
            ...
            if labels[r_pick] * labels[s_pick] > 0:  # Check if labels are the same, then compute admissible interval
        ...
        final_obj = (1 / 2) * (dual_vars @ gram @ dual_vars) - np.sum(dual_vars)  # Compute final objective function
        opt_dict = {
            "InitObj": 0,
            "FinalOpt": final_obj,
            "Iterations": it,
